...
Elm.Pi.make = function (_elm) {
   ...
   var genPoint = function (s) {
      return function () {
         var x1 = A2($Random.generate,
         probability,
         s);
         var y1 = A2($Random.generate,
         probability,
         $Basics.snd(x1));
         return {ctor: "_Tuple2"
                ,_0: {_: {}
   ...
   F2(function (x,pair) {
      return genPoint($Basics.snd(pair));
   }),
   {ctor: "_Tuple2"
   ,_0: {_: {},x: 0,y: 0}
   ,_1: $Random.initialSeed(6)},
   $Time.every(100 * $Time.millisecond));
   var signalPoint = A2($Signal.map,
   $Basics.fst,
   signalPointSeed);
   ...
   var asText$ = function (s) {
      return $Text.leftAligned($Text.monospace($Text.fromString(s)));
   };
   var piApprox = function (st) {
      return function () {
         var two = $Basics.toFloat($Basics.fst($Basics.snd(st)));
         var one = $Basics.toFloat($Basics.fst($Basics.fst(st)));
         return one / two;
      }();
   };
   var pointCircle = F2(function (color,
   pt) {
      return function () {
         var py = pt.y * 150.0;
         var px = pt.x * 150.0;
         ...
         35) < 0 && (_U.cmp(-50.0,
         py) < 0 && _U.cmp(py,
         50.0) < 0))) ? A2($Graphics$Collage.move,
         {ctor: "_Tuple2",_0: px,_1: py},
         A2($Graphics$Collage.filled,
         $Color.black,
         $Graphics$Collage.circle(3))) : A2($Graphics$Collage.move,
         {ctor: "_Tuple2",_0: px,_1: py},
         A2($Graphics$Collage.filled,
         color,
         $Graphics$Collage.circle(3)));
      }();
   });
   var pointsToCircles = F2(function (color,
   listpoints) {
      return function () {
         var mapping = F2(function (size,
         list) {
            return A2($List.map,
            function (pt) {
               return A2(pointCircle,
               color,
               pt);
            },
            list);
         ...
         20,
         listpoints);
         var firsttwenty = A2($List.take,
         20,
         listpoints);
         var scale = 150.0;
         return A2($Basics._op["++"],
         A2(mapping,3,firsttwenty),
         A2($List.map,
         function (x) {
         ...
         {case "_Tuple2":
            return A3($Graphics$Collage.collage,
              ...
              _L.fromArray([A2($Graphics$Collage.filled,
                           $Color.lightGrey,
                           $Graphics$Collage.square(300))
                           ,A2($Graphics$Collage.filled,
                           $Color.white,
                           $Graphics$Collage.circle(150))
                           ,A2($Graphics$Collage.move,
                           {ctor: "_Tuple2"
                           ,_0: -95
                           ,_1: -160},
                           $Graphics$Collage.toForm(asText$("Approximation")))
                           ,A2($Graphics$Collage.move,
                           {ctor: "_Tuple2"
                           ,_0: -100
                           ,_1: -180},
                           $Graphics$Collage.toForm(asText$("Samples")))
                           ,A2($Graphics$Collage.move,
                           {ctor: "_Tuple2"
                           ,_0: 80
                           ,_1: -160},
                           $Graphics$Collage.toForm($Text.asText(piApprox(st))))
                           ,A2($Graphics$Collage.move,
                           {ctor: "_Tuple2"
                           ,_0: 80
                           ,_1: -180},
                           $Graphics$Collage.toForm($Text.asText($Basics.fst($Basics.fst(st)) + $Basics.fst($Basics.snd(st)))))]),
              A2($Basics._op["++"],
              A2(pointsToCircles,
              $Color.blue,
   ...
   st) {
      ...
      1) < 0 ? {ctor: "_Tuple2"
               ,_0: {ctor: "_Tuple2"
                    ,_0: $Basics.fst($Basics.fst(st)) + 1
                    ,_1: A2($List._op["::"],
                    pt,
                    $Basics.snd($Basics.fst(st)))}
               ,_1: $Basics.snd(st)} : {ctor: "_Tuple2"
                                       ,_0: $Basics.fst(st)
                                       ,_1: {ctor: "_Tuple2"
                                            ,_0: $Basics.fst($Basics.snd(st)) + 1
                                            ,_1: A2($List._op["::"],
                                            pt,
                                            $Basics.snd($Basics.snd(st)))}};
   });
